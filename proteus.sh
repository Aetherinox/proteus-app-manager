...
server {
    ...
    root $tor_website_path;
    server_name $tor_website_addr;
...
EOF
    ...
    sleep 1
    if [ -z "${OPT_DEV_NULLRUN}" ]; then
        sudo systemctl enable tor >> $LOGS_FILE 2>&1
        sudo systemctl restart tor >> $LOGS_FILE 2>&1
    fi
    echo -e "[ ${STATUS_OK} ]"
